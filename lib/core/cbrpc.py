    ...
    def get_conn(self):
        with self.lock:
            if self.cur_connections >= self.max_connections:
                ...
                logger.info("RpcConnectionPool status: name=%s|max=%s|cur=%s|qsize=%s",\
                        self.name, self.max_connections, self.cur_connections, self.pools.qsize())
                return conn
...
def get_rpc_conn(rpc_name):
    ...
    if pool is None:
        ...
        pool = RpcConnectionPool(host=confs["host"], port=confs["port"], max_connections=confs["max_connections"], rpc_name=rpc_name)
